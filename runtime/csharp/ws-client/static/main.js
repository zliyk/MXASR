...
// 不自动检测地址，使用HTML中设置的默认地址
// 如果需要自动检测，只在http/https模式下进行，避免file://模式出错
var currentUrl = window.location.href;
if (currentUrl.startsWith('http://') || currentUrl.startsWith('https://')) {
    // 仅在HTTP/HTTPS模式下尝试自动检测
    ...
    if (localport) {
        now_ipaddress = now_ipaddress.replace(localport,"9095"); // 使用9095端口
    } else {
        // 如果没有端口号，在域名后添加端口
        var domainEnd = now_ipaddress.indexOf('/', 8); // 查找域名后的第一个斜杠
        if (domainEnd !== -1) {
            now_ipaddress = now_ipaddress.substring(0, domainEnd) + ':9095' + now_ipaddress.substring(domainEnd);
        } else {
            now_ipaddress = now_ipaddress + ':9095';
        }
    }
